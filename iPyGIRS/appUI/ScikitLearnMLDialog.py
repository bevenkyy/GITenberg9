...
class ScikitLearnMLDialog(QDialog, Ui_ScikitLearnMLDialog):
    ...
    def connect_singal_solt(self):
        ...
        self.selectTaskTypeComboBox.currentIndexChanged.connect(self.selectTaskTypeComboBoxCurrentIndexChanged)
    ...
    def init_ui_element(self):
        ...
        self.ui_element = {"selectSamplePage":{"operationLogoLabel":get_pixmap("import_data"),
                            ...
                            "optimizeEstimatorPage":{"operationLogoLabel":get_pixmap("optimize_model"),
                                                 "operationTitleLabel":"优化模型",
                                                 "operationDescriptionLabel":"更改参数或数据集，重新训练以调优模型。",
                                                 "stepLogoLabel":get_pixmap("info_tip"),
                                                 "stepTipLabel":"点击任一优化按钮来优化模型，选择新测试数据集点击“测试”按钮以评估模型性能，点击“下一步”以继续操作！"},
        ...
        #
        self.operationLogoLabel.setText(self.qTranslate("ScikitLearnMLDialog", ""))
        self.stepLogoLabel.setText(self.qTranslate("ScikitLearnMLDialog", ""))
    ...
    def init_estimator_save_params(self):
        #
        ...
        #
        self.isSaveBestEstimatorToFileLineEdit.setText(self.estimator_name + "_Best_Parameters_Estimator.json")
        self.isSaveEstimatorDataToExcelFileNameLineEdit.setText(self.estimator_name + "_Estimator_Data.xlsx")
        self.isSaveTrainingDataToWorkingSheetNameLineEdit.setText(self.estimator_name + "_Training")
        self.isSaveCVDataToWorkingSheetNameLineEdit.setText(self.estimator_name + "_CrossValidation")
        self.isSaveTestDataToWorkingSheetNameLineEdit.setText(self.estimator_name + "_Test")
        #
    ...
    def update_basic_params(self):
        ...
        if self.task_type == "Classification":
            if estimator_name_text == "逻辑回归":
                self.estimator_name = "LogisticRegression"
            elif estimator_name_text == "支持向量机":
                self.estimator_name = "SVC"
            elif estimator_name_text == "神经网络":
                self.estimator_name = "MLPClassifier"
            elif estimator_name_text == "决策树":
                self.estimator_name = "DecisionTreeClassifier"
            elif estimator_name_text == "随机森林":
                self.estimator_name = "RandomForestClassifier"
            else:
                self.estimator_name = "GradientBoostingClassifier"
        elif self.task_type == "Cluster":
            if estimator_name_text == "K-Means聚类":
                self.estimator_name = "KMeans"
        else:
            if estimator_name_text == "线性回归":
                self.estimator_name = "LinearRegression"
            elif estimator_name_text == "岭回归":
                self.estimator_name = "Ridge"
            elif estimator_name_text == "套索回归":
                self.estimator_name = "Lasso"
            elif estimator_name_text == "支持向量机":
                self.estimator_name = "SVR"
            elif estimator_name_text == "神经网络":
                self.estimator_name = "MLPRegressor"
            elif estimator_name_text == "决策树":
                self.estimator_name = "DecisionTreeRegressor"
            elif estimator_name_text == "随机森林":
                self.estimator_name = "RandomForestRegressor"
            else:
                # GBDT algorithm
                self.estimator_name = "GradientBoostingRegressor"
        #
        ...
        #
        if self.selectParameterTypeComboBox.currentText() == "默认参数":
            self.params_type = "default"
        elif self.selectParameterTypeComboBox.currentText() == "调试参数":
            self.params_type = "tuning"
        else:
            self.params_type = "custom"
        #
    ...
    def selectTrainingValidationSamplePushButtonClicked(self):
        ...
        if file_path != "":
            ...
            #
            self.training_val_sample_hist_chart = HistgramChart(self.training_cv_samples[:,-1], 
            ...
            self.sampleStatisticChartGridLayout.addWidget(self.training_val_sample_hist_chart,0,0,1,1)
            #
    ...
    def selectTestSamplePushButtonClicked(self):
        ...
        if file_path != "":
            ...
            #
            self.test_sample_hist_chart = HistgramChart(self.test_samples[:,-1].astype(np.float64), title="Test Samples Histogram")
            self.sampleStatisticChartGridLayout.removeWidget(self.test_sample_axis_chart)
            self.sampleStatisticChartGridLayout.addWidget(self.test_sample_hist_chart,0,1,1,1)
            #
            self.qSetting.setValue("lastFileDir", os.path.dirname(file_path))

    # =========================================================Set Model Parameter Page=========================================================

    def selectTaskTypeComboBoxCurrentIndexChanged(self):
        #
        ...
        #
        valid_classifier = ["逻辑回归", "支持向量机", "神经网络", "决策树", "随机森林", "梯度提升树"]
        valid_cluster = ["K-Means聚类",]
        valid_regressor = ["线性回归","岭回归","套索回归", "支持向量机", "神经网络", "决策树", "随机森林", "梯度提升树"]
        self.selectEstimatorComboBox.clear()
        #
        if self.task_type == "Classification":
            self.selectEstimatorComboBox.addItems(valid_classifier)
        elif self.task_type == "Cluster":
            self.selectEstimatorComboBox.addItems(valid_cluster)
        else:
            self.selectEstimatorComboBox.addItems(valid_regressor)
        #
    ...
    def estimatorEvaluationChartPushButtonClicked(self):
        if self.test_samples is None:
            if self.params_type == "tuning":
                data = [[self.trained_data[2][:,0], self.trained_data[2][:,1], "Training", "Measurement", "Prediction"],
                        [self.cv_data[2][:, 0], self.cv_data[2][:, 1], "Corss-Validation", "Measurement", "Prediction"]]
            else:
                data = [[self.trained_data[2][:,0], self.trained_data[2][:,1], "Training", "Measurement", "Prediction"]]
        else:
            if self.params_type == "tuning":
                data = [[self.trained_data[2][:,0], self.trained_data[2][:,1], "Training", "Measurement", "Prediction"],
                        [self.cv_data[2][:, 0], self.cv_data[2][:, 1], "Corss-Validation", "Measurement", "Prediction"],
                        [self.test_data[2][:,0], self.test_data[2][:,1], "Test", "Measurement", "Prediction"]]
            else:
                data = [[self.trained_data[2][:,0], self.trained_data[2][:,1], "Training", "Measurement", "Prediction"],
                        [self.test_data[2][:,0], self.test_data[2][:,1], "Test", "Measurement", "Prediction"]]
        #
    ...
    def trainEstimatorPushButtonClicked(self):
        ...
        self.fit_thread = RunFitEstimatorThread(self.training_cv_samples, self.test_samples,
                                                self.task_type, self.estimator_name, 
...
class RunFitEstimatorThread(QtCore.QThread):
    ...
    def run(self):
        ...
        with Manager() as process_manager:
            ...
            #
            estimator = ScikitLearnML(self.task_type, self.estimator_name, self.params_type, 
