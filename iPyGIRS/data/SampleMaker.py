...
def __sampling_index(start_index, stop_index, group_samples_count):
    '''
    '''
...
def stratified_sampling(data_array, trainval_samples_count, test_samples_count, group_samples_count, 
    ...
    for i in range(0, data_array.shape[0], step):
        if i + step <= data_array.shape[0]:
            sampling_index_tuple = __sampling_index(i, i + step - 1, group_samples_count)
    ...
    test_remaining_samples_index = __sampling_index(0, remaining_samples.shape[0] - 1, test_remaining_samples_count)
