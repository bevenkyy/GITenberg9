...
class RunThread(QtCore.QThread):
    ...
    def run(self):
        ...
        for i, (img_filename, pixel_value_filename) in enumerate(zip(self.image_file_list, self.pixel_value_file_list)):
            #
            geotiff_info = PixelValueExtractor.query_geotiff_info(img_filename)
            ...
            for j in range(1, band_count + 1):
                col_title.append("Band_" + str(j))
            #
            pixel_value_array = np.zeros([coordinate_count, len(col_title)], dtype = np.float64)
            pixel_value_array[:, 0:2] = lonlat_coordinate_array
            #
            for k in range(1, band_count + 1):
                #
                img_arr = RasterImgIO.read_single_band(img_filename, k)
                tmp_pixel_value_list = []
                #
                for coordinates in colrow_coordinate_array:
                    try:
                    ...
                    #
                    statusPercent = int(round((i + 1)/image_file_count * 100.0))
                    if statusPercent > 99:
                        statusPercent = 99
                    #
                    self.updateFinishedStatus_signal.emit((image_file_count, i + 1, statusPercent))
                #
                pixel_value_array[:, k + 1] = tmp_pixel_value_list
            #
